...
- TikTok direct auto-posting requires `TIKTOK_POSTING_MODE=direct`,
  ...
  `PUBLIC_TO_EVERYONE`.
- TikTok `TIKTOK_POSTING_MODE=upload` is diagnostic only. It creates inbox
  drafts using `video.upload`, which require manual finish and URL logging, so
  it is excluded from the active no-manual-posting plan.
