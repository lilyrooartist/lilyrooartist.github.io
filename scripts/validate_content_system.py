...
def validate_generated_outputs(failures):
    ...
    if BRAND_GROWTH_PREFLIGHT.exists():
        ...
        if (
            ...
            and int(summary.get("scheduler_satisfied_count", summary.get("scheduler_would_post_count") or 0) or 0) == expected_count
    ...
    if PROMO_OPERATIONS_PACKET.exists():
        ...
        if int((scheduled_packet.get("summary") or {}).get("approval_blocker_count") or 0) and scheduled_checked_change_count > 0:
            if (
                ...
                and all((action.get("context") or {}).get("approval_apply_manifest") == scheduled_apply_manifest for action in scheduled_batch_actions)
                and all((action.get("context") or {}).get("pre_apply_checklist") == (scheduled_apply_manifest.get("pre_apply_checklist") or []) for action in scheduled_batch_actions)
                and all((action.get("context") or {}).get("post_apply_evidence") == (scheduled_apply_manifest.get("post_apply_evidence") or []) for action in scheduled_batch_actions)
                and all((action.get("context") or {}).get("apply_guardrails") == (scheduled_apply_manifest.get("guardrails") or []) for action in scheduled_batch_actions)
                and all(((action.get("context") or {}).get("approval_review_runbook") or {}).get("steps") == (scheduled_runbook.get("steps") or []) for action in scheduled_batch_actions)
                and all((action.get("context") or {}).get("review_runbook_step_count") == len(scheduled_runbook.get("steps") or []) for action in scheduled_batch_actions)
                and all((action.get("context") or {}).get("review_checklist_count") == len(scheduled_runbook.get("review_checklist") or []) for action in scheduled_batch_actions)
            ...
            if all(
                ...
                and ((action.get("context") or {}).get("approval_impact") or {}).get("approval_apply_manifest") == scheduled_apply_manifest
                and ((action.get("context") or {}).get("approval_impact") or {}).get("pre_apply_checklist") == (scheduled_apply_manifest.get("pre_apply_checklist") or [])
                and ((action.get("context") or {}).get("approval_impact") or {}).get("post_apply_evidence") == (scheduled_apply_manifest.get("post_apply_evidence") or [])
                and ((action.get("context") or {}).get("approval_impact") or {}).get("apply_guardrails") == (scheduled_apply_manifest.get("guardrails") or [])
    ...
    if SCHEDULED_APPROVAL_PACKET.exists():
        ...
        apply_manifest = approval_packet.get("approval_apply_manifest") or {}
        ...
        review_runbook = approval_packet.get("approval_review_runbook") or {}
        review_steps = review_runbook.get("steps") or []
        ...
        if (
            ...
            and apply_manifest.get("status") == ("ready_for_human_review" if checked_rows else "blocked")
            and apply_manifest.get("ready_ids") == [row.get("id") for row in checked_rows]
            and apply_manifest.get("held_ids") == [row.get("id") for row in blocked_rows]
            and apply_manifest.get("apply_scope") == "checked_batch"
            and apply_manifest.get("preview_command") == summary.get("checked_batch_preview_command")
            and apply_manifest.get("apply_after_review_command") == summary.get("checked_batch_apply_command")
            and apply_manifest.get("expected_change_count") == checked_batch_effect.get("change_count")
            and apply_manifest.get("expected_effect") == checked_batch_effect
            and [item.get("id") for item in apply_manifest.get("ready_rows") or []] == [row.get("id") for row in checked_rows]
            and [item.get("id") for item in apply_manifest.get("held_rows") or []] == [row.get("id") for row in blocked_rows]
            and all({"copy", "destination_links", "media_asset", "platform_readiness"} <= set(item.get("required_review") or []) for item in apply_manifest.get("ready_rows") or [])
            and all(item.get("failed_check_names") for item in apply_manifest.get("held_rows") or [])
            and any("Run the preview command" in item for item in apply_manifest.get("pre_apply_checklist") or [])
            and any("data/scheduled_posts.csv" in item for item in apply_manifest.get("post_apply_evidence") or [])
            and any("does not approve" in item for item in apply_manifest.get("guardrails") or [])
            and review_runbook.get("status") == docket.get("status")
            and review_runbook.get("ready_ids") == [row.get("id") for row in checked_rows]
            and review_runbook.get("blocked_ids") == [row.get("id") for row in blocked_rows]
            and review_runbook.get("ready_count") == len(checked_rows)
            and review_runbook.get("blocked_count") == len(blocked_rows)
            and review_runbook.get("manual_dispatch_ready_count") == len([row for row in checked_rows if row.get("manual_dispatch")])
            and len(review_checklist) == len(checked_rows)
            and [item.get("id") for item in review_checklist] == [row.get("id") for row in checked_rows]
            and all(
                item.get("human_review_required") is True
                and item.get("copy_block")
                and item.get("asset_url")
                and item.get("destination_links")
                and item.get("preview_command")
                and item.get("post_approval_next_step")
                and {"copy", "destination_links", "media_asset", "platform_readiness"} <= {check.get("item") for check in item.get("checklist", [])}
                for item in review_checklist
            )
            and [step.get("order") for step in review_steps] == [1, 2, 3, 4, 5]
            and review_steps[1].get("command") == summary.get("checked_batch_preview_command")
            and review_steps[2].get("command") == summary.get("checked_batch_apply_command")
            and "review-only" in (review_runbook.get("guardrail") or "")
    ...
    if MANUAL_DISTRIBUTION_PACKET.exists():
        ...
        if (
            ...
            and completion_manifest.get("preserved_public_url_count") == len([row for row in url_template_rows if (row.get("public_url") or "").strip()])
            and "preserved by id" in (completion_manifest.get("url_template_preservation") or "")
            ...
            and all((manifest_row.get("public_url") or "") == (template_row.get("public_url") or "") for manifest_row, template_row in zip(completion_manifest.get("url_entry_rows") or [], url_template_rows))
    ...
    if MANUAL_POSTING_CLIPBOARD.exists():
        ...
        if (
            ...
            and summary.get("posting_bundle_count") == len(cards)
    ...
    if EXPERIMENT_RESULT_CLIPBOARD.exists():
        ...
        if (
            ...
            and (clipboard.get("summary") or {}).get("post_log_measurement_handoff_count") == len(handoff_rows)
    ...
    if MANUAL_METRIC_PACKET.exists():
        ...
        if (
            ...
            and import_manifest.get("waiting_csv_rows") == [row.get("csv_row") for row in rows if not row.get("ready_to_import")]
    ...
    if PROMO_ENGINE_STATUS.exists():
        ...
        next_format_runbook = growth_goal.get("next_format_evidence_runbook") or {}
        format_ladder = growth_goal.get("repeatable_format_ladder") or {}
        ladder_steps = format_ladder.get("steps") or []
        ...
        if (
            ...
            and top_candidate_formats == experiment_formats
            and all(candidate.get("format_basis") == "experiment_format" for candidate in top_candidates)
            ...
            and format_ladder.get("needed_winner_count") == max((winner_readiness.get("winner_count_target") or 0) - (winner_readiness.get("ready_candidate_count") or 0), 0)
            and format_ladder.get("minimum_measured_posts_per_format") == winner_readiness.get("minimum_measured_posts_per_format")
            and len(ladder_steps) == len(top_candidates)
            ...
            and (
                ...
                or (
                    ...
                    and "measured result values" in (next_format_runbook.get("guardrail") or "")
                )
            )
        ...
        scheduled_packet = json.loads(SCHEDULED_APPROVAL_PACKET.read_text(encoding="utf-8")) if SCHEDULED_APPROVAL_PACKET.exists() else {}
        scheduled_status = kpi.get("scheduled_approval") or {}
        if (
            scheduled_status.get("available") is True
            and scheduled_status.get("source_path") == "data/scheduled_approval_packet.json"
            and scheduled_status.get("summary") == (scheduled_packet.get("summary") or {})
            and scheduled_status.get("approval_decision_manifest") == (scheduled_packet.get("approval_decision_manifest") or {})
            and scheduled_status.get("approval_apply_manifest") == (scheduled_packet.get("approval_apply_manifest") or {})
            and scheduled_status.get("approval_review_runbook") == (scheduled_packet.get("approval_review_runbook") or {})
        ):
            ok("promo engine status mirrors scheduled approval apply manifest")
        else:
            fail("promo_engine_status.json missing scheduled approval apply manifest", failures)
    ...
    if (
        ...
        and "SITE_LYRICS_CLICK_PATTERN" in worker_text
        and "site-lyrics-" in worker_text
    ...
    if SCHEDULED_APPROVAL_REPORT.exists():
        ...
        if "Scheduled Approval Packet" in scheduled_approval_report_text and "Approval Docket" in scheduled_approval_report_text and "Approval Apply Manifest" in scheduled_approval_report_text and "Pre-Apply Checklist" in scheduled_approval_report_text and "Post-Apply Evidence" in scheduled_approval_report_text and "Approval Review Runbook" in scheduled_approval_report_text and "Ready Row Checklist" in scheduled_approval_report_text and "Review Queue" in scheduled_approval_report_text and "Guardrails" in scheduled_approval_report_text:
    ...
    if EXPERIMENT_RESULT_CLIPBOARD_REPORT.exists():
        ...
        if "Experiment Result Clipboard" in result_clipboard_report and "Metric Cards" in result_clipboard_report and "Measurement Priorities" in result_clipboard_report and "Post-Log Measurement Handoff" in result_clipboard_report and "Handoff rows" in result_clipboard_report and "Missing Public URLs" in result_clipboard_report and "Preview import" in result_clipboard_report and "Wide rows ready to import" in result_clipboard_report and "Wide entry instruction" in result_clipboard_report and "Wide-ready fields" in result_clipboard_report and "Evidence sources" in result_clipboard_report and "Collection checklist" in result_clipboard_report and "Guardrails" in result_clipboard_report:
...
def validate_simple_admin_dashboard(failures):
    ...
    checks = {
        ...
        "recent activity column": 'id="simple-recent-activity"' in text and "Recent Activity &amp; Results" in text and "A plain-English record" in text,
        "upcoming activity column": 'id="simple-upcoming-activity"' in text and "Upcoming Activities" in text and "What will happen next automatically" in text,
        "two equal columns": ".simple-columns{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr)" in text,
        "four status cards across top": ".simple-status-strip{display:grid;grid-template-columns:repeat(4,minmax(180px,1fr))" in text,
        "clear status wording": "Analog Myth promotion is running automatically." in text and "Next automatic activity:" in text,
        "clear result wording": "what published, what link was saved, and what result check is ready" in text,
        "upcoming wording is scheduled": "plus the follow-up checks after posts go live" in text,
...
def validate_analog_myth_launch_share(failures):
    ...
    required_redirect = [
        ...
        'id="track-art"',
        'const DEFAULT_ALBUM_ART = "/assets/albums/analog-myth/art/03-analog-myth.jpg";',
        'art: "/assets/albums/analog-myth/art/04-spilling-the-tea.jpg"',
        'art: "/assets/albums/analog-myth/art/08-the-power-of-light.png"',
        "artImage.src = track.art || DEFAULT_ALBUM_ART",
        "artImage.alt = track.title",
...
def validate_lyrics_discovery_metadata(failures):
    ...
    for path in pages:
        ...
        required_tokens = [
            ...
            'name="twitter:card" content="summary_large_image"',
            "lyrics-growth-cta",
            f"p=site-lyrics-{path.stem}-album&amp;to=album",
            f"p=site-lyrics-{path.stem}-listen&amp;to=listen",
            f"p=site-lyrics-{path.stem}-echo&amp;to=echo",
        ]
        missing = [token for token in required_tokens if token not in text]
        if missing:
