...
def check_social_launch_pack(results: list[dict], require_store_links: bool) -> None:
    ...
    add_result(
        results,
        "Launch promo pack includes manual Spotify URL fallback commands",
        "python3 scripts/run_analog_myth_launch.py --live --spotify-url VERIFIED_SPOTIFY_ALBUM_URL" in text
    ...
    add_result(results, "Launch promo pack documents optional Apple and YouTube Music validation", "Apple Music or YouTube Music URLs" in text and "validated as `Analog Myth` by `Lily Roo`" in text)
    add_result(results, "Launch promo pack preserves optional URL args through next commands", "optional URL arguments are preserved" in text and "next_commands" in text)
