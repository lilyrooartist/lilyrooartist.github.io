# Song Backstory Cards (Organized)

## Brain Rot
- **Card ID:** LR-SONG-BRAINROT-01
- **Theme tags:** feed fatigue, overstimulation, satire, dissociation
- **Narrative angle:** "The song where scrolling became a horror-comedy"
- **Anecdote seeds:**
  - You can feel too many things and still go numb.
  - Brain Rot is the laugh right before the panic attack.
- **Lyric links:** add to `11_LYRIC_BACKSTORY_INDEX.csv`
- **Status:** draft (needs confirmed lyric excerpts)

## Analog Myth
- **Card ID:** LR-SONG-ANALOGMYTH-01
- **Theme tags:** time distortion, nonlinearity, memory
- **Narrative angle:** "Clocks measure numbers, not grief"
- **Linked anecdotes:** LR-ANEC-001..004

## Guards Down
- **Card ID:** LR-SONG-GUARDSDOWN-01
- **Theme tags:** decision pressure, screen truth, courage, no-undo stakes
- **Narrative angle:** "No soft version of brave"
- **Linked anecdotes:** LR-ANEC-022..025

## I Learned It All in Fifteen Seconds
- **Card ID:** LR-SONG-15SEC-01
- **Theme tags:** empathy overload, digital intimacy, fragmented witnessing
- **Narrative angle:** "A thousand lives through a small glowing screen"
- **Linked anecdotes:** LR-ANEC-005..008
